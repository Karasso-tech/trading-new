...
class TestSectorExposure:
    ...
    def test_position_with_no_real_risk_adds_no_risk_but_still_holds_money(self, temp_db):
        """A stop already trailed above entry has no downside left, so it must
        not contribute to the RISK split.

        It does still hold money, though, and as of 2026-08-30 the group shows
        that separately. This test used to assert the group vanished entirely,
        which was an assertion about a side effect rather than about the intent:
        a position that cannot lose anything is still capital sitting in one
        correlated bet, and rule 20 was written about exactly that -- ~84% of
        the real book in one place.
        """
        ...
        # the risk split is untouched: all of it is JPM's
        assert exposure["Information Technology"]["risk_usd"] == 0.0
        assert exposure["Financials"]["pct_of_swing_book"] == pytest.approx(1.0)
        # and the money is still visible where it actually is
        assert exposure["Information Technology"]["capital_usd"] == pytest.approx(208.62 * 75)
        assert exposure["Information Technology"]["pct_of_swing_capital"] > 0.4
...
class TestShadowRowsPerSetup:
    """2026-08-30. The shadow book's unique index was (idea_id, checked_date).

    That is not merely imprecise, it is a wall. Rule 5 requires every report to
    carry a Primary and an Alternate, and on any given night those are two
    different simulated trades on the same idea -- which the two-column key
    could physically hold only one of. So the Alternate was never scored, and
    the book has been learning from half of what the system produces.
    """

    def _idea(self, ticker="ABC"):
        persistence.save_thesis(
            ticker=ticker, status="pending", source="SCREENER_v3",
            primary_setup={"type": "Breakout", "trigger": 100.0, "stop": 97.0,
                            "atr_at_build": 2.0,
                            "targets": [{"price": 106.0, "pct": "40", "status": "pass"}]},
            alternate_setup={"type": "Pullback", "trigger": 95.0, "stop": 92.0,
                              "atr_at_build": 2.0},
        )
        with persistence._db() as conn:
            row = conn.execute(
                "SELECT id FROM ideas WHERE ticker=? ORDER BY id DESC LIMIT 1", (ticker,)
            ).fetchone()
        return row["id"] if row else None

    def test_both_setups_fit_on_the_same_night(self, temp_db):
        idea_id = self._idea()
        common = dict(ticker="ABC", checked_date="2026-08-30", price=101.0,
                       hypothetical_trigger_fired=True, max_favorable_excursion=1.0,
                       max_adverse_excursion=-1.0, idea_id=idea_id)
        persistence.record_shadow_outcome(**common, setup_side="primary")
        persistence.record_shadow_outcome(**common, setup_side="alternate")

        with persistence._db() as conn:
            sides = [r["setup_side"] for r in conn.execute(
                "SELECT setup_side FROM shadow_outcomes WHERE idea_id=? ORDER BY setup_side",
                (idea_id,))]
        assert sides == ["alternate", "primary"]

    def test_the_same_setup_twice_in_one_night_is_still_refused(self, temp_db):
        # The duplicate this index was created for in the first place: a
        # scheduled run firing twice wrote two rows that disagreed about whether
        # the trigger had fired.
        idea_id = self._idea()
        common = dict(ticker="ABC", checked_date="2026-08-30", price=101.0,
                       hypothetical_trigger_fired=True, max_favorable_excursion=1.0,
                       max_adverse_excursion=-1.0, idea_id=idea_id, setup_side="primary")
        persistence.record_shadow_outcome(**common)
        with pytest.raises(sqlite3.IntegrityError):
            persistence.record_shadow_outcome(**common)

    def test_rows_default_to_primary_so_history_is_not_relabelled(self, temp_db):
        idea_id = self._idea()
        persistence.record_shadow_outcome(
            ticker="ABC", checked_date="2026-08-30", price=101.0,
            hypothetical_trigger_fired=False, max_favorable_excursion=None,
            max_adverse_excursion=None, idea_id=idea_id)
        with persistence._db() as conn:
            row = conn.execute(
                "SELECT setup_side, is_fully_closed FROM shadow_outcomes WHERE idea_id=?",
                (idea_id,)).fetchone()
        assert row["setup_side"] == "primary"
        assert row["is_fully_closed"] == 0


class TestIsFullyClosed:
    """`resolution` cannot say whether a trade is finished.

    trade_sim sets it to "target_1" the moment the first tranche sells and
    leaves it there while the runner keeps running, so a live position and a
    finished one are spelled identically. Every average taken over
    "resolution != open" has been mixing marks-to-last-close into closed
    results -- including, for one afternoon, an analysis of this very book that
    reported 136 closed trades where the honest number was much smaller.
    """
    ...
    def _row(self, resolution):
        ...
        persistence.record_shadow_outcome(
            ticker="ABC", checked_date="2026-08-30", price=101.0,
            hypothetical_trigger_fired=True, max_favorable_excursion=1.0,
            max_adverse_excursion=-1.0, resolution=resolution)
        with persistence._db() as conn:
            return conn.execute(
                "SELECT is_fully_closed FROM shadow_outcomes ORDER BY id DESC LIMIT 1"
            ).fetchone()["is_fully_closed"]

    @pytest.mark.parametrize("resolution", ["stop", "runner_trailed", "giveback"])
    def test_the_backfill_marks_an_ended_position_closed(self, temp_db, resolution):
        # The backfill runs on every init_db, so a row written without the field
        # still gets the right answer where the resolution can only mean one thing.
        self._row(resolution)
        persistence.init_db()
        with persistence._db() as conn:
            assert conn.execute(
                "SELECT is_fully_closed FROM shadow_outcomes ORDER BY id DESC LIMIT 1"
            ).fetchone()["is_fully_closed"] == 1

    @pytest.mark.parametrize("resolution", ["target_1", "target_2", "open", "never_fired"])
    def test_a_tranche_sale_is_not_treated_as_the_end(self, temp_db, resolution):
        self._row(resolution)
        persistence.init_db()
        with persistence._db() as conn:
            assert conn.execute(
                "SELECT is_fully_closed FROM shadow_outcomes ORDER BY id DESC LIMIT 1"
            ).fetchone()["is_fully_closed"] == 0

class TestTheBookCountsTradesNotNights:
    """The shadow table is append-only by design: one row per idea per setup per
    night, each a true record of what was known that evening. That is worth
    keeping, and it is not a dataset.

    Counting those rows counts nights. UPS stopped out on 2026-07-17 and went on
    collecting an identical row every night until it had 18; CRDO had 18, TYL
    had 18. An analysis of this book earlier the same day reported "104 closed
    trades" where the honest number was 13.
    """

    def _idea(self, ticker):
        persistence.save_thesis(
            ticker=ticker, status="pending", source="SCREENER_v3",
            primary_setup={"type": "Breakout", "trigger": 100.0, "stop": 97.0,
                            "atr_at_build": 2.0,
    ...
    def _night(self, ticker, idea_id, date, r, closed=False, side="primary"):
        persistence.record_shadow_outcome(
            ticker=ticker, checked_date=date, price=101.0,
            hypothetical_trigger_fired=True, max_favorable_excursion=1.0,
            max_adverse_excursion=-1.0, idea_id=idea_id, setup_side=side,
            r_multiple_planned=r, resolution="stop" if closed else "open",
            is_fully_closed=int(closed))

    def test_eighteen_nights_of_one_trade_count_as_one_trade(self, temp_db):
        idea_id = self._idea("UPS")
        for day in range(1, 19):
            self._night("UPS", idea_id, f"2026-08-{day:02d}", -1.383, closed=(day == 18))

        book = persistence.get_shadow_trades()
        assert len(book["trades"]) == 1
        assert book["closed"] == 1
        # and the one kept is the LAST night, not the first
        assert book["trades"][0]["checked_date"] == "2026-08-18"

    def test_nothing_is_deleted_to_achieve_that(self, temp_db):
        # The nightly record is evidence of what was known each evening. The
        # grouping happens at read time; the rows stay.
        idea_id = self._idea("UPS")
        for day in range(1, 6):
            self._night("UPS", idea_id, f"2026-08-{day:02d}", -1.0)
        with persistence._db() as conn:
            assert conn.execute("SELECT COUNT(*) FROM shadow_outcomes").fetchone()[0] == 5
        assert len(persistence.get_shadow_trades()["trades"]) == 1

    def test_the_two_setups_of_one_idea_are_two_trades(self, temp_db):
        idea_id = self._idea("MU")
        self._night("MU", idea_id, "2026-08-01", 2.0, side="primary")
        self._night("MU", idea_id, "2026-08-01", -1.0, side="alternate")

        book = persistence.get_shadow_trades()
        assert len(book["trades"]) == 2
        assert {t["setup_side"] for t in book["trades"]} == {"primary", "alternate"}

    def test_rows_too_old_to_group_are_reported_not_dropped(self, temp_db):
        # 93 real rows predate idea_id and carry only a ticker, which is not an
        # identity when a symbol can be screened three times. Silently dropping
        # them would understate the book; silently counting them would double it.
        self._idea("ABC")
        persistence.record_shadow_outcome(
            ticker="ABC", checked_date="2026-07-01", price=100.0,
            hypothetical_trigger_fired=False, max_favorable_excursion=None,
            max_adverse_excursion=None)          # no idea_id

        book = persistence.get_shadow_trades()
        assert book["ungrouped"] == 1
        assert book["trades"] == []

    def test_only_finished_trades_when_asked_for_only_finished(self, temp_db):
        a, b = self._idea("AAA"), self._idea("BBB")
        self._night("AAA", a, "2026-08-01", 2.0, closed=True)
        self._night("BBB", b, "2026-08-01", 0.4, closed=False)

        book = persistence.get_shadow_trades(include_open=False)
        assert [t["ticker"] for t in book["trades"]] == ["AAA"]
        assert book["open"] == 0


class TestAFinishedTradeIsNotScoredAgain:
    def _idea(self, ticker="UPS"):
        persistence.save_thesis(
            ticker=ticker, status="pending", source="SCREENER_v3",
            primary_setup={"type": "Breakout", "trigger": 100.0, "stop": 97.0,
                            "atr_at_build": 2.0, "targets": []})
    ...
    def test_a_live_trade_is_still_a_candidate(self, temp_db):
        idea_id = self._idea()
        persistence.record_shadow_outcome(
            ticker="UPS", checked_date="2026-08-01", price=100.0,
            hypothetical_trigger_fired=True, max_favorable_excursion=1.0,
            max_adverse_excursion=-1.0, idea_id=idea_id, resolution="open",
            is_fully_closed=0)
        assert [c["ticker"] for c in persistence.get_shadow_candidates()] == ["UPS"]

    def test_a_finished_trade_drops_out(self, temp_db):
        # This is what stops the 18 rows. A position that is out does not change
        # again, so re-simulating it nightly adds nights, not information.
        #
        # BOTH setups have to be done, as of 2026-08-30: a build whose Primary
        # has finished while its Alternate has never been scored still has work
        # left on it, and dropping it there was how the Alternate stayed
        # invisible in the first place.
        idea_id = self._idea()
        for side in ("primary", "alternate"):
            persistence.record_shadow_outcome(
                ticker="UPS", checked_date="2026-08-01", price=100.0,
                hypothetical_trigger_fired=True, max_favorable_excursion=1.0,
                max_adverse_excursion=-1.0, idea_id=idea_id, setup_side=side,
                resolution="stop", is_fully_closed=1)
        assert persistence.get_shadow_candidates() == []

    def test_a_build_stays_while_its_alternate_is_unscored(self, temp_db):
        idea_id = self._idea()
        persistence.record_shadow_outcome(
            ticker="UPS", checked_date="2026-08-01", price=100.0,
            hypothetical_trigger_fired=True, max_favorable_excursion=1.0,
        ...
        assert [c["ticker"] for c in remaining] == ["UPS"]
        assert remaining[0]["finished_setup_sides"] == ["primary"]

class TestMissingDataIsNamedNotDropped:
    """Rule 19-21 figures are about to decide whether an order may be placed, so
    the one shape of error they must never have is looking complete when they
    are not.

    Both of these paths existed and had never been exercised: nothing on the
    live book is missing a stop today. That is precisely why they were worth
    fixing before the numbers start blocking anything.
    """

    def test_a_position_with_no_stop_does_not_quietly_vanish_from_heat(self, temp_db):
        persistence.set_equity(100_000.0)
        _open_swing("JPM", entry_price=100.0, current_stop=95.0, qty=100)   # risk 500
        _open_swing("NVDA", entry_price=200.0, current_stop=None, qty=50)   # unknown

        heat = persistence.get_portfolio_heat()
        assert heat["heat_usd"] == pytest.approx(500.0)
        assert heat["complete"] is False
        assert [u["ticker"] for u in heat["unmeasurable"]] == ["NVDA"]

    def test_a_complete_book_says_so(self, temp_db):
        persistence.set_equity(100_000.0)
        _open_swing("JPM", entry_price=100.0, current_stop=95.0, qty=100)
        heat = persistence.get_portfolio_heat()
        assert heat["complete"] is True
        assert heat["unmeasurable"] == []

    def test_allocation_says_which_prices_it_used(self, temp_db):
        persistence.set_equity(100_000.0)
        _open_swing("JPM", entry_price=100.0, current_stop=95.0, qty=100)

        at_cost = persistence.get_allocation_drift()
        assert at_cost["valued_at"] == "entry"
        assert at_cost["unvalued"] == ["JPM"]

        at_market = persistence.get_allocation_drift(prices={"JPM": 150.0})
        assert at_market["valued_at"] == "market"
        assert at_market["unvalued"] == []
        # 50% higher mark -> a bigger swing share, which is the whole point:
        # a book valued at cost drifts further from the truth the better it does
        assert at_market["swing_pct_actual"] > at_cost["swing_pct_actual"]

    def test_one_missing_quote_makes_the_whole_ratio_a_mixture(self, temp_db):
        persistence.set_equity(100_000.0)
        _open_swing("JPM", entry_price=100.0, current_stop=95.0, qty=100)
        _open_swing("NVDA", entry_price=200.0, current_stop=190.0, qty=50)

        drift = persistence.get_allocation_drift(prices={"JPM": 150.0})
        assert drift["valued_at"] == "entry"      # not "market" -- it is mixed
        assert drift["unvalued"] == ["NVDA"]
