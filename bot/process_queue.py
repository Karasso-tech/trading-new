...
_FILLED_RE = re.compile(r"^/filled\s+(\S+)\s+([\d.]+)\s+(\d+)(?:\s+(starter|full))?\s*$", re.IGNORECASE)
...
def _handle_filled(update_id: int, text: str) -> bool:
    ...
    # Which setup actually triggered: primary_setup by default (the common case) --
    # stated explicitly in the confirmation so a wrong pick is visible and correctable,
    # never silently assumed. Trigger levels are sometimes descriptive text, not a
    # clean number, so matching against alternate_setup automatically isn't reliable.
    ...
    resp = send_text(
        ...
        f"סטופ: {entry_setup.get('stop')} (מ-Primary setup — ודא שזה נכון)\n"
        f"{risk_line}"
...
_SCREENER_PROMPT_TEMPLATE = (
    ...
    "movement potential, rubric_grade, market_regime, rejection_reasons and max_allowed_decision, plus "
    ...
    "(including stop_basis_level and atr_at_build), rubric_grade, market_regime, market_regime_formula "
    "and rejection_reasons VERBATIM. Copy data.freshness verbatim. Add your own alternate_setup, "
...
_PLAYBOOK_PROMPT_TEMPLATE = (
    ...
    "invent a new stop from generic chart structure as if starting fresh. **Do not work the trailed stop out by hand: run `python bot\\trail_stop.py TICKER` for each open position.** It applies STRATEGY_v3's own method (highest daily low clearing the 0.7x ATR noise floor, rule 24's 0.15x ATR buffer underneath it, measured against the position's FROZEN atr_at_build, never moving down) and returns stop_should_be plus the level behind it. Use that number and quote its stop_basis_level. Start from current_stop and only "
...
_SYNTAX_HINTS = {
    ...
    "/filled": "/filled TICKER price qty starter|full",
