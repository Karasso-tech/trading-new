...
const server = new McpServer(
  ...
  {
    instructions: `TradingView MCP — 80 tools for reading and controlling a live TradingView chart open in Chrome.
...
Launch: tv_launch → auto-detect and start a dedicated CDP-enabled Chrome window on TradingView on any platform
Panes: pane_list, pane_set_layout (s, 2h, 2v, 4, 6, 8), pane_focus, pane_set_symbol
Tabs: tab_list, tab_new, tab_close, tab_switch

Calendars (no chart tab needed -- direct TradingView JSON endpoints):
- calendar_get_earnings → next/last earnings date per ticker. Use this before ever
  claiming an earnings date is "verified" -- never state one from memory.
- calendar_get_economic → upcoming/recent US macro events (CPI, PPI, NFP, FOMC, Fed
  rate decisions, unemployment, GDP by default). Use for the "same-day Fed/CPI/NFP
  release" event check.
...
// Start HTTP bridge for Python app (port 9223)
startHttpServer();
