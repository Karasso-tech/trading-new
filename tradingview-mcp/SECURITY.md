...
This project connects to a locally running TradingView chart tab (open in a dedicated Chrome profile) via Chrome DevTools Protocol on `localhost:9222`. Security concerns in scope include:
...
- Only run the debug Chrome window with `--remote-debugging-port=9222` on localhost
...
- Keep this dedicated Chrome profile logged in only to TradingView -- don't sign into other sensitive accounts in it, since anything in that browser session is reachable via CDP
- Keep Chrome and your Node.js installation up to date
