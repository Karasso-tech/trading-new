...
def compute_shadow_metrics(bars: list[dict], trigger: float, since_date: str) -> ShadowMetrics:
...
#
# 20 trading days is not tuned and is not claimed to be: it is one trading month,
# and it is double MONITOR_v2's own TRIGGER_STALE_TRADING_DAYS, so an idea is
# called dead only well after the system had already stopped treating its trigger
# as fresh. Same honest-caveat posture as rule 4's 0.7x ATR noise floor -- a first
# draft, written down, worth re-checking against real history rather than trusted
# forever.
NEVER_FIRED_EXPIRY_TRADING_DAYS = 20
...
def run_one(thesis: dict) -> Optional[int]:
    """Fetches, scores, and persists one candidate. Returns the new
    shadow_outcomes row id, or None if this ticker had nothing scorable (no
    numeric trigger, no date_built), the fetch failed, or this build was already
    scored today -- all logged, never raised, so a batch run keeps going.
    ...
    The idea_id it carries is written onto the result, so a symbol screened
    ...
    trigger = _extract_trigger(thesis.get("primary_setup"))
    since_date = (thesis.get("date_built") or thesis.get("built_at") or "")[:10]
    if trigger is None or not since_date:
        print(f"SKIP {ticker}: no numeric trigger or date_built on stored primary_setup")
        return None
    try:
    ...
    plan = extract_plan(thesis.get("primary_setup"))
    ...
    try:
        ...
        print(f"SKIP {ticker}: build {idea_id} already scored on {today}")
        return None
    r_text = "n/a" if sim.r_multiple_planned is None else f"{sim.r_multiple_planned:+.2f}R"
    print(f"OK {ticker}: {resolution}, planned={r_text}, "
