...
def _lint_pending_trigger_wording(decision: dict, result: LintResult) -> None:
    """A prose trigger has to be prose for a reason, and say so the same way
    every time.

    Rule 5 permits a setup whose level has not formed yet. Read across 28 real
    pending Alternates on 2026-08-31, that permission was doing two jobs at
    once: 25 of the 28 NAMED A PRICE inside the sentence. ANET's read "ממתין
    לירידה לאזור SMA50 (159.91)" -- 159.91 is the trigger, sitting in prose
    because the sentence also explained that no order was ready yet.

    Those 25 were invisible to everything mechanical: the shadow book could not
    simulate them, the rubric could not score them, the stale-trigger check
    could not see them. Not because the level was missing, but because it was
    written in the wrong field.

    So two findings, and they are different problems:
      * a level is named in the text -> put the number in `trigger`, keep the
        wording as the note beside it
      * no level is named -> use one of setup_types.PENDING_TRIGGER_PHRASES, so
        "how often does this actually happen" is a count rather than a reading
        exercise. Same reasoning as rule 29's fixed rejection vocabulary, which
        exists because one situation came back in five spellings.
    """
    for label, key in (("Primary", "primary_setup"), ("Alternate", "alternate_setup")):
        setup = decision.get(key)
        if not isinstance(setup, dict):
            continue
        trigger = setup.get("trigger")
        if _clean_number(trigger) is not None:
            continue          # already a number -- nothing to say
        if not isinstance(trigger, str) or not trigger.strip():
            continue          # absent entirely; _lint_structural_completeness owns that

        levels = setup_types.levels_named_in_text(trigger)
        if levels:
            result.findings.append(LintFinding(
                check="level_named_in_prose_trigger",
                message_he=(f"⚠️ ל-{label} יש רמה בטקסט ({', '.join(levels[:3])}) אבל שדה "
                             f"הטריגר אינו מספר. המספר צריך לשבת בשדה, והמשפט לידו — "
                             f"אחרת שום בדיקה אוטומטית לא רואה את הסטאפ הזה (כלל 5)"),
                detail={"setup": label, "levels": levels, "trigger_text": trigger},
            ))
        elif not setup_types.is_pending_trigger_phrase(trigger):
            result.findings.append(LintFinding(
                check="pending_trigger_wording",
                message_he=(f"ℹ️ ל-{label} אין רמה עדיין, וזה מותר — אבל הניסוח חופשי. "
                             f"אחד מהמשפטים הקבועים במקום, כדי שאפשר יהיה לספור כמה "
                             f"פעמים זה קורה: {' · '.join(setup_types.PENDING_TRIGGER_PHRASES)}"),
                detail={"setup": label, "trigger_text": trigger},
            ))


def _lint_target_scan_ran(decision: dict, result: LintResult) -> None:
    """Rule 7/12: a setup with a real entry and a real stop must show the RESULT
    of a target scan, even when that result is "nothing qualified".

    Rule 7 says it outright -- "Every setup shown gets this analysis
    independently; Alternate is not exempt just because it's second or still
    pending" -- and records the miss that produced it: MU, ONDS and ANET each
    had the Primary's targets fully analysed and the Alternate left as
    trigger/stop only, with no scan attempted at all.

    It happened 23 more times. Found 2026-08-31 by reading the shadow book, once
    the Alternate started being simulated: 23 Alternates carried a numeric
    trigger and a numeric stop and produced no target. Twenty of those turned
    out to be honest -- the scan ran, found levels, and none passed rule 3's
    gate, which is exactly what `checkpoints` records. Three had neither
    targets nor checkpoints, which is not an answer, it is a missing step.

    So the check is for the SCAN, not for a target: either list being non-empty
    means the work was done. It never judges which level should have qualified
    -- same consistency-check-only shape as rules 18/23/27's own backstops."""
    for label, key in (("Primary", "primary_setup"), ("Alternate", "alternate_setup")):
        setup = decision.get(key)
        if not isinstance(setup, dict):
            continue
        if _clean_number(setup.get("trigger")) is None or _clean_number(setup.get("stop")) is None:
            continue          # nothing to scan from yet -- rule 14's pending path
        if setup.get("targets") or setup.get("checkpoints"):
            continue
        result.findings.append(LintFinding(
            check="target_scan_missing",
            message_he=(f"⚠️ ל-{label} יש טריגר וסטופ אבל אין אף יעד ואף צ'קפוינט — "
                         f"סריקת היעדים לא הותירה שום פלט (כללים 7 ו-12). "
                         f"\"לא נמצא יעד\" הוא פלט; ריק הוא שלב חסר"),
            detail={"setup": label, "trigger": setup.get("trigger"),
                    "stop": setup.get("stop")},
        ))
...
def _lint_rubric_grade_gate(decision: dict, result: LintResult) -> None:
    ...
    grade = decision_policy.resolve_grade(decision).grade
    dec = decision.get("decision")
...
def _lint_rubric_recompute(decision: dict, result: LintResult) -> None:
    """CONSISTENCY_RULES.md rule 27's promised backstop.

    The recompute itself is decision_policy.resolve_grade, because it is a GATE:
    deliver_report refuses to ship an order on any verdict it blocks, and lint
    never blocks anything. This function's job is to say out loud, in the
    warning block and in the stored lint record, WHY a report was refused --
    naming both numbers when they disagree, rather than leaving a reader to
    infer it from a decision that quietly came out weaker than expected.

    A report with no rubric_inputs is recorded as skipped as well as flagged:
    the field is newer than the oldest stored theses, and telling those two
    situations apart later matters more than one extra warning line now."""
    verdict = decision_policy.resolve_grade(decision)
    if not verdict.blocked:
        return
    if verdict.reason_key in ("inputs_missing", "inputs_malformed"):
        result.skipped.append(
            f"rubric: {verdict.reason_key} -- the grade could not be recomputed and checked"
        )
    if verdict.reason_key == "mismatch":
        result.findings.append(LintFinding(
            check="rubric_grade_mismatch",
            message_he=(f"\u26a0\ufe0f \u05d4\u05e6\u05d9\u05d5\u05df \u05d1\u05d3\u05d5\u05d7 \u05d4\u05d5\u05d0 {verdict.stated} \u05d0\u05d1\u05dc \u05d4\u05d7\u05d9\u05e9\u05d5\u05d1 \u05de\u05d4\u05de\u05e1\u05e4\u05e8\u05d9\u05dd \u05e9\u05d4\u05d3\u05d5\u05d7 \u05e2\u05e6\u05de\u05d5 "
                         f"\u05de\u05e6\u05d9\u05d2 \u05e0\u05d5\u05ea\u05df {verdict.recomputed} (\u05db\u05dc\u05dc 27)"),
            detail={"stated_grade": verdict.stated, "recomputed_grade": verdict.recomputed,
                    "recomputed_score": verdict.score},
        ))
        return
    if verdict.reason_key == "names_conflict":
        result.findings.append(LintFinding(
            check="rubric_grade_names_conflict",
            message_he=(f"\u26a0\ufe0f \u05d4\u05d3\u05d5\u05d7 \u05de\u05e6\u05d9\u05d9\u05df \u05e9\u05e0\u05d9 \u05e6\u05d9\u05d5\u05e0\u05d9\u05dd \u05e9\u05d5\u05e0\u05d9\u05dd: rubric_grade="
                         f"{decision.get('rubric_grade')} \u05de\u05d5\u05dc grade={decision.get('grade')} (\u05db\u05dc\u05dc 27)"),
            detail={"rubric_grade": decision.get("rubric_grade"), "grade": decision.get("grade")},
        ))
        return
    if verdict.reason_key in ("inputs_missing", "inputs_malformed"):
        result.findings.append(LintFinding(
            check="rubric_inputs_unusable",
            message_he=(f"\u26a0\ufe0f {verdict.reason_he} (\u05db\u05dc\u05dc 27) \u2014 \u05dc\u05dc\u05d0 \u05d1\u05d3\u05d9\u05e7\u05d4 \u05db\u05d6\u05d0\u05ea "
                         f"\u05d0\u05d9\u05df \u05e4\u05e7\u05d5\u05d3\u05d4"),
            detail={"reason": verdict.reason_key, "stated_grade": verdict.stated},
        ))
...
def _lint_decision_word(decision: dict, result: LintResult) -> None:
    ...
    if not has_target:
        ...
        why = ("הסטופ לא עומד על שום שפל בגרף — הוא מרחק קבוע של 2 ATR, "
               "וזה מספר בלי מקור (כלל 1)")
    elif regime in decision_policy.BLOCKING_REGIMES:
        why = f"מצב השוק '{regime}' חוסם פקודה (כלל 18)"
    elif grade not in decision_policy.KNOWN_GRADES:
        # Not the same sentence as a D. "No grade was written down" and "the
        # grade was written down and it is bad" are two different facts, and the
        # first one used to print as "ציון רובריקה None", which reads like a
        # letter rather than a missing field.
        why = "אין ציון רובריקה בדוח הזה בכלל — פקודה בלי ציון היא פקודה שאיכותה לא נבדקה (כלל 27)"
    else:
...
def lint_screener_decision(decision: dict) -> LintResult:
    ...
    _lint_target_scan_ran(decision, result)
    _lint_pending_trigger_wording(decision, result)
    _lint_regime_gate(decision, result)
    _lint_rubric_grade_gate(decision, result)
    _lint_rubric_recompute(decision, result)
