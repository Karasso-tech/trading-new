...
export async function getOhlcv({ count, summary, before } = {}) {
  const limit = Math.min(count || 100, MAX_OHLCV_BARS);
  // `before` (unix seconds) is a vendored addition (not upstream) for pagination:
  // when provided, anchor the window to end at the last bar strictly before this
  // timestamp instead of the live edge (bars.lastIndex()). This lets a caller walk
  // backward through history 500 bars at a time by re-passing the oldest `time`
  // seen in the previous page. Without it, this tool can only ever see the most
  // recent `count` bars, which silently caps daily history at ~2 years.
  const beforeTs = before != null ? Number(before) : null;
  let data;
  try {
    data = await evaluate(`
      (function() {
        ...
        var beforeTs = ${beforeTs === null ? 'null' : beforeTs};
        if (beforeTs !== null) {
          // Binary search for the last index whose bar time is < beforeTs.
          var lo = bars.firstIndex(), hi = bars.lastIndex(), candidate = lo - 1;
          while (lo <= hi) {
            var mid = (lo + hi) >> 1;
            var v = bars.valueAt(mid);
            if (v && v[0] < beforeTs) { candidate = mid; lo = mid + 1; }
            else { hi = mid - 1; }
          }
          end = candidate;
        }
        ...
        return {
          bars: result, total_bars: bars.size(), source: 'direct_bars',
          buffer_first_index: bars.firstIndex(), buffer_last_index: bars.lastIndex(),
          reached_start_of_buffer: start <= bars.firstIndex(),
        };
      })()
    `);
  } catch { data = null; }
  ...
  return {
    success: true, bar_count: data.bars.length, total_available: data.total_bars, source: data.source, bars: data.bars,
    reached_start_of_buffer: data.reached_start_of_buffer,
  };
}
