 ...
 * Controls TradingView tabs (open in Chrome) via CDP and standard browser keyboard shortcuts.
 */
import { getClient, evaluate } from '../connection.js';

const CDP_HOST = 'localhost';
// 2026-07-30 full-system checkup: was a bare hardcoded literal independent of
// connection.js's own port -- see connection.js's getCdpPort() for why this
// reads the same env var set by core/health.js's launch() instead.
const getCdpPort = () => Number(process.env.TV_MCP_CDP_PORT) || 9222;

/**
 * List all open chart tabs (CDP page targets).
 */
export async function list() {
  const resp = await fetch(`http://${CDP_HOST}:${getCdpPort()}/json/list`);
...
export async function closeTab() {
  ...
  if (before.tab_count <= 1) {
    throw new Error('Cannot close the last tab. Use tv_launch to restart the TradingView Chrome window instead.');
  }
...
export async function switchTab({ index }) {
  ...
  try {
    const resp = await fetch(`http://${CDP_HOST}:${getCdpPort()}/json/activate/${target.id}`);
