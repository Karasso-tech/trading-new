...
export async function setSymbol({ symbol, _deps }) {
  ...
  // 150ms, was 500ms (2026-08-04). This delay exists to stop waitForChartReady
  // polling during the window where the chart API has already accepted the new
  // symbol but the loading spinner has not appeared yet -- two consecutive
  // "clean" polls in that window would return ready while the OLD symbol's bars
  // are still loaded. Measured: the spinner appears 12-68ms after the setter
  // returns, so 150ms clears the observed worst case by more than 2x while
  // returning 350ms per symbol switch. Do not remove it entirely: at
  // POLL_INTERVAL=50ms two consecutive polls span only 100ms and could both
  // land before the spinner shows.
  await evaluateAsync(`
    (function() {
      var chart = ${CHART_API};
