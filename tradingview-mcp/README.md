...
Personal AI assistant for your TradingView charts, open in Chrome. Connects Claude Code to a TradingView chart tab running in a dedicated, CDP-enabled Chrome window via Chrome DevTools Protocol for AI-assisted chart analysis, Pine Script development, and workflow automation.

> [!WARNING]
> **This tool is not affiliated with, endorsed by, or associated with TradingView Inc.** It interacts with your locally running TradingView chart (open in Chrome) via Chrome DevTools Protocol. Review the [Disclaimer](#disclaimer) before use.

> [!IMPORTANT]
> **Requires a valid TradingView subscription.** This tool does not bypass or circumvent any TradingView paywall or access control. It reads from and controls the TradingView chart tab already open in Chrome on your machine.
...
> [!CAUTION]
> This tool accesses undocumented internal TradingView APIs via the Chrome debug interface. These can change or break without notice in any TradingView update. Pin your Chrome version if stability matters to you.

## How It Works (and why it's safe to run)

This tool does not connect to TradingView's servers, modify any TradingView files, or intercept any network traffic. It communicates exclusively with a TradingView chart tab open in your locally running Chrome via Chrome DevTools Protocol (CDP) — a standard debugging interface built into all Chromium-based applications by Google, including VS Code, Slack, and Discord.

The debug port is disabled by default and must be explicitly enabled by you using a standard Chromium flag (`--remote-debugging-port=9222`). Nothing happens without that deliberate step. `tv_launch` and the `scripts/launch_tv_debug*` scripts always use a dedicated Chrome profile (`--user-data-dir`) for this, separate from your everyday browsing profile, so it never touches your regular Chrome windows/tabs.

## What This Tool Does Not Do

- Connect to TradingView's servers or APIs
- Store, transmit, or redistribute any market data
- Work without a valid TradingView subscription and a signed-in TradingView chart tab
...
- How structured tool APIs (MCP) can bridge LLMs and stateful browser-based financial applications
...
- **Google Chrome** with a TradingView account (paid subscription required for real-time data)
- **Node.js 18+**
...
- **Launch TradingView** — auto-detect Chrome and launch it in debug mode, in a dedicated profile, from any platform

## Install with Claude Code

Paste this into Claude Code and it will handle the rest:

> Install the TradingView MCP server. Clone https://github.com/tradesdontlie/tradingview-mcp.git, run npm install, add it to my MCP config at ~/.claude/.mcp.json, and launch a debug Chrome window on TradingView. Then verify the connection with tv_health_check.
...
TradingView must be open in a Chrome window running with Chrome DevTools Protocol enabled on port 9222 (use a dedicated profile, not your everyday Chrome — the scripts/tool below already do this).
...
```bash
/path/to/chrome --remote-debugging-port=9222 --user-data-dir="/path/to/tradingview-mcp-chrome-profile" https://www.tradingview.com/chart/
```

**Or use the MCP tool** (auto-detects your Chrome install):
...
The `tv stream` commands poll your locally running TradingView chart tab (in Chrome) at regular intervals via Chrome DevTools Protocol on localhost.
...
```bash
# Requires TradingView open in Chrome with --remote-debugging-port=9222
npm test
```
...
```
Claude Code  ←→  MCP Server (stdio)  ←→  CDP (port 9222)  ←→  Chrome (tradingview.com/chart, dedicated profile)
```
...
2. TradingView's Terms of Use **restrict automated data collection, scraping, and non-display usage** of their platform and data. This tool uses Chrome DevTools Protocol to programmatically interact with a TradingView chart tab open in Chrome, which may conflict with those terms.
...
4. This tool **must not be used** for, including but not limited to:
   ...
   - Connecting to TradingView's servers or infrastructure (all access is via the locally running Chrome tab)
5. The streaming functionality monitors your locally running TradingView chart tab only. It does not connect to TradingView's servers or extract data from TradingView's infrastructure.
