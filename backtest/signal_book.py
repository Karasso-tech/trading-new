...
def main() -> None:
    ...
    out_json = ROOT / f"signal_book_{args.seed}.json"
    out_json.write_text(json.dumps(signals, indent=1), encoding="utf-8")
    out_csv = ROOT / f"signal_book_{args.seed}.csv"
...
def finish(position, ctx, bars) -> dict:
    ...
    mfe = max(b["high"] for b in bars[i:j + 1])
    mae = min(b["low"] for b in bars[i:j + 1])
