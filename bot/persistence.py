...
-- Risk-cap overrides (2026-08-30). Portfolio heat became a real block on that
-- date -- the first figure in rules 19-21 that stops an order rather than
-- describing one -- and a block with no way past it is a block that gets worked
-- around outside the system, where nothing records it.
--
-- Deliberately narrow, because a broad override is the same as no cap:
--   * one ticker, not a global switch
--   * one named breach, so waving heat through says nothing about anything else
--   * a written reason, stored, never judged
--   * a short life, so it cannot be set once and forgotten
--   * consumed on use, so it authorises one order and not a habit
CREATE TABLE IF NOT EXISTS risk_overrides (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    ticker      TEXT NOT NULL,
    breach      TEXT NOT NULL,          -- which cap: 'heat'
...
def _backfill_shadow_is_fully_closed(conn: sqlite3.Connection) -> None:
    """Set is_fully_closed on rows written before the column existed.

    Only from resolutions that can only mean "nothing left": the original stop
    took the rest, the trail took the rest, or a give-back rule closed it. Those
    three end a position by definition.

    Deliberately NOT from target_1/target_2. Those say a tranche sold; the
    runner may still be running, and which of the two it was is exactly the
    thing the old rows cannot tell us. Leaving them 0 understates how many
    closed trades exist and never overstates it, which is the right direction
    for a field whose whole purpose is to keep live marks out of closed-trade
    averages. score_shadow writes the real value from the simulation on every
    row from here on.

    Guarded so it runs once: rows already carrying 1 are left alone."""
    conn.execute(
        "UPDATE shadow_outcomes SET is_fully_closed = 1 "
        "WHERE is_fully_closed = 0 AND resolution IN ('stop', 'runner_trailed', 'giveback')"
    )
...
def init_db() -> None:
    with _db() as conn:
        ...
        _ensure_columns(conn, "thesis", {
            "rubric_grade": "TEXT",
            # 2026-08-30: the six numbers the grade was computed from, as JSON.
            # Kept on the thesis, not only in the run's decision file, because
            # the shadow book has to be able to re-score the SAME setup against
            # the entry it really got -- and it cannot do that from a letter.
            "rubric_inputs": "TEXT",
            "market_regime_at_build": "TEXT",
            "planned_qty": "INTEGER",
            "drop_reason": "TEXT",
        ...
        _ensure_columns(conn, "ideas", {
            # 2026-08-30: the six numbers the grade was computed from, as JSON.
            # Needed here and not only in the run's decision file, because the
            # shadow book re-scores the SAME setup against the entry it really
            # got, and a letter cannot be re-scored.
            "rubric_inputs": "TEXT",
        })
        _ensure_columns(conn, "shadow_outcomes", {
            ...
            # 2026-08-30. WHICH of the thesis's two setups this row simulated.
            # Rule 5 requires every report to carry a Primary and an Alternate,
            # and only the Primary has ever been scored -- so the shadow book
            # has been learning from half of what the system produces, with no
            # field able to say so. Existing rows are all Primary, which is what
            # the default records rather than assumes.
            "setup_side": "TEXT NOT NULL DEFAULT 'primary'",
            # 2026-08-30. Does this row describe a trade with nothing left in
            # it? `resolution` cannot answer that: trade_sim sets "target_1" the
            # moment the first tranche sells and leaves it there while the
            # runner keeps going, so a still-open trade and a finished one are
            # spelled the same. Every average taken over "resolution != open"
            # has therefore been mixing live marks into closed results.
            "is_fully_closed": "INTEGER NOT NULL DEFAULT 0",
            # 2026-08-30. An idea that never fired used to record "did not fire"
            # and nothing else -- every field describing what price did was NULL.
            # So the book could say how many ideas started, and nothing at all
            # about why the rest did not: missed by a cent or missed by a mile
            # were stored identically. That difference is the most useful thing
            # here for making entries more precise, and it is exactly what
            # waiting longer cannot supply -- a year of waiting still only says
            # fired or did not.
            # 2026-08-30. A grade at build and a grade at the moment price
            # actually triggered are two different claims about the same setup,
            # and mixing them makes it impossible to say which one failed.
            # Rule 27 already draws this line for the live system -- R:R is
            # measured from the entry the user would ACTUALLY get -- and the
            # shadow book had no field for the second number at all.
            "grade_at_fire": "TEXT",
            "rr_at_fire": "REAL",
            "closest_approach_pct": "REAL",    # how far short of the trigger, % (0 = touched)
            "closest_approach_atr": "REAL",    # the same gap in the setup's own ATR
            "closest_approach_date": "TEXT",
            "move_without_entry_pct": "REAL",  # what the stock did while we stood aside
        })
        _backfill_shadow_is_fully_closed(conn)
        # One shadow row per SETUP per idea per day. Runs were firing more than
        # once on some nights (found 2026-08-07: two identical XLF rows on
        # 08-04, and two NVDA rows on 08-02 that disagreed on whether the
        # trigger had fired), and nothing in the schema forbade the repeat.
        # Partial index -- rows backfilled before idea_id existed stay NULL and
        # are not constrained.
        #
        # setup_side joined the key on 2026-08-30. Without it the index is not
        # merely imprecise, it is a wall: an idea's Primary and its Alternate
        # are two different simulated trades on the same night, and the old
        # two-column key could physically hold only one of them. The old index
        # is dropped rather than left beside the new one, since keeping it would
        # enforce exactly the constraint being removed.
        conn.execute("DROP INDEX IF EXISTS idx_shadow_one_per_idea_per_day")
        conn.execute(
            "CREATE UNIQUE INDEX IF NOT EXISTS idx_shadow_one_per_setup_per_day "
            "ON shadow_outcomes(idea_id, setup_side, checked_date) WHERE idea_id IS NOT NULL"
        )
...
def get_portfolio_heat() -> dict:
    ...
    with _db() as conn:
        positions = conn.execute(
            "SELECT id, ticker, entry_price, current_stop, qty FROM positions WHERE status='open'"
        ).fetchall()
        heat_usd = 0.0
        unmeasurable = []
        for p in positions:
            ...
            if remaining <= 0:
                continue
            # A position with no stop on file does not carry zero risk. It
            # carries UNKNOWN risk, and the two must not print the same.
            #
            # This used to `continue` past it, so the position vanished from the
            # total silently -- the one shape of error a risk figure must never
            # have, because the number still looked complete. Named here instead,
            # and `complete` says out loud whether the total can be trusted as a
            # total. Nothing on the live book is in this state today; the path
            # existed and had never been exercised.
            if p["current_stop"] is None:
                unmeasurable.append({"ticker": p["ticker"], "why": "no stop on file",
                                      "qty": remaining})
                continue
            ...
            if risk_per_share > 0:
                ...
                # The stop sits ABOVE the entry -- a profitable trade whose stop
                # has trailed past breakeven. Genuinely zero risk from here, and
                # a real state, not a gap.
                continue
    ...
    return {
        ...
        "heat_pct": heat_pct,
        "cap_pct": cap_pct,
        # False when at least one open position could not be measured, so a
        # reader can tell "4.8% of the cap" from "at least 4.8%, and one
        # position could not be counted".
        "complete": not unmeasurable,
        "unmeasurable": unmeasurable,
        "breached": (heat_pct is not None and heat_pct > cap_pct),
    }


def get_allocation_drift(prices: Optional[dict] = None) -> dict:
    ...
    with _db() as conn:
        ...
        for r in rows:
            ...
            # At market where a price was handed in, at cost otherwise -- and
            # which of the two is stated rather than left to be assumed. A book
            # valued at entry drifts further from the truth the better it does,
            # so an allocation figure that silently uses cost reads most wrong
            # exactly when the position matters most.
            mark = (prices or {}).get(r["ticker"])
            if mark is None:
                unvalued.append(r["ticker"])
                mark = r["entry_price"]
            value = mark * remaining
            if r["sleeve"] == "core":
                core_usd += value
                if r["ticker"] == "SPY":
                    spy_usd += value
    ...
    if not equity:
        return {
            ...
            "valued_at": "unknown", "unvalued": unvalued,
        }
    ...
    return {
        ...
        # "market" only when EVERY open position had a price to mark against.
        # One missing quote makes the whole ratio a mixture, and a mixture
        # labelled "market" is worse than one labelled honestly.
        "valued_at": "market" if (prices and not unvalued) else "entry",
        "unvalued": unvalued,
    }


OVERRIDE_HOURS = 12   # long enough to place the order it was written for,
                       # short enough that it cannot quietly become the rule


def add_risk_override(ticker: str, breach: str, reason: str) -> int:
    """Record permission to place ONE order past ONE cap, for a short while.

    The reason is stored and never judged -- same posture as rule 28's
    size_reduction_reason. What makes this safe is not the quality of the
    sentence, it is that a deliberate act was required and is on the record."""
    if not (reason or "").strip():
        raise ValueError("an override needs a written reason")
    now = datetime.now(timezone.utc)
    with _db() as conn:
        cur = conn.execute(
            "INSERT INTO risk_overrides (ticker, breach, reason, created_at, expires_at) "
            "VALUES (?, ?, ?, ?, ?)",
            (ticker.upper(), breach, reason.strip(), now.isoformat(),
             (now + timedelta(hours=OVERRIDE_HOURS)).isoformat()),
        )
        return cur.lastrowid


def find_risk_override(ticker: str, breach: str) -> Optional[dict]:
    """An unused, unexpired override for this exact ticker and breach, or None."""
    with _db() as conn:
        row = conn.execute(
            "SELECT * FROM risk_overrides WHERE ticker=? AND breach=? AND used_at IS NULL "
            "AND expires_at > ? ORDER BY id DESC LIMIT 1",
            (ticker.upper(), breach, datetime.now(timezone.utc).isoformat()),
        ).fetchone()
    return dict(row) if row else None


def consume_risk_override(override_id: int) -> None:
    """Spend it. One override authorises one order; a second order needs a
    second decision."""
    with _db() as conn:
        conn.execute("UPDATE risk_overrides SET used_at=? WHERE id=? AND used_at IS NULL",
                      (_now(), override_id))
...
def get_sector_exposure() -> dict:
    ...
    with _db() as conn:
        ...
        for r in rows:
            ...
            # Two different concentrations, and the older one is not the one a
            # person means by "too much in tech" (2026-08-30).
            #
            # RISK concentration is how much of the money that can be LOST to
            # stops sits in one group. That is what this function measured, and
            # it is genuinely useful -- but a tight stop makes a huge position
            # look small in it. CAPITAL concentration is how much money is
            # actually parked in the group, which is what a correlated drawdown
            # or a gap through every stop at once actually hits.
            #
            # The strategy review that produced rule 20 found ~84% of the real
            # book in one correlated bet. That was a CAPITAL observation, and
            # the rule it produced has been checked against risk ever since.
            capital = r["entry_price"] * remaining
            group_capital[group] = group_capital.get(group, 0.0) + capital
            swing_capital_total += capital

            if r["current_stop"] is None:
                # Unknown risk, not zero risk. Named, never dropped -- the same
                # correction get_portfolio_heat needed.
                unmeasurable.append({"ticker": r["ticker"], "why": "no stop on file"})
                continue
    ...
    if not swing_total and not swing_capital_total:
        return {}
    groups = sorted(set(group_risk) | set(group_capital))
    return {
        group: {
            "risk_usd": group_risk.get(group, 0.0),
            # Unchanged meaning and unchanged name, so every existing reader
            # and every stored disclosure keeps saying what it always said.
            "pct_of_swing_book": (group_risk.get(group, 0.0) / swing_total) if swing_total else None,
            # The money actually parked in the group. See the comment in the
            # loop for why this is a different question from the one above, and
            # why it is the one rule 20 was really written about.
            "capital_usd": group_capital.get(group, 0.0),
            "pct_of_swing_capital": ((group_capital.get(group, 0.0) / swing_capital_total)
                                      if swing_capital_total else None),
            # Positions in this group whose risk could not be measured at all.
            # A group can be 40% of the capital and show 0% of the risk purely
            # because its one position has no stop on file.
            "unmeasurable": [u for u in unmeasurable
                              if (sector_map.get_sector_group(u["ticker"]) or "unclassified") == group],
        }
        for group in groups
    }
...
def _record_idea(conn, ticker: str, status: str, source: Optional[str], sleeve: Optional[str],
                 ...
                 rubric_inputs: Optional[dict] = None) -> int:
    ...
    cursor = conn.execute(
        ...
        "decision, rubric_grade, rubric_inputs, market_regime_at_build, planned_qty, rejection_reasons, "
        ...
        (
            ...
            decision, rubric_grade,
            json.dumps(rubric_inputs, ensure_ascii=False) if rubric_inputs else None,
            market_regime_at_build, planned_qty,
...
def save_thesis(ticker: str, status: str, source: str, primary_setup: dict,
                ...
                rubric_grade: Optional[str] = None, rubric_inputs: Optional[dict] = None,
                market_regime_at_build: Optional[str] = None,
    ...
    with _db() as conn:
        ...
        conn.execute(
            ...
            "alternate_setup, position, rubric_grade, rubric_inputs, market_regime_at_build, planned_qty, "
            ...
            "rubric_grade=excluded.rubric_grade, rubric_inputs=excluded.rubric_inputs, "
            "market_regime_at_build=excluded.market_regime_at_build, "
        ...
        idea_id = _record_idea(
            ...
            rubric_grade=rubric_grade, rubric_inputs=rubric_inputs,
            market_regime_at_build=market_regime_at_build,
...
def get_pnl_positions() -> list[dict]:
    """Every position this system ever recorded -- open and closed -- reduced to
    the two money figures a "trades vs Core" split is made of: dollars already
    banked (from this position's own exits rows) and shares still held (which
    only a live price can value).

    Dollars, not R-multiples. R is the right unit for judging whether the
    system works (/journal, bot/score_shadow.py) and the wrong unit for
    answering "what is the broker's one blended number actually made of" --
    the question this feeds. The two must never be mixed in one column.

    Commissions are subtracted exactly where they were really recorded -- the
    entry commission once per position, each exit's own commission with that
    exit -- and are simply absent where they never were, rather than estimated
    from a rate nobody entered.

    `sleeve` comes from get_sleeve(), the same unconditional SPY/QQQ rule
    get_open_positions() and get_allocation_drift() already use, so "Core"
    here can never disagree with "Core" anywhere else.

    No live fetch, same posture as get_open_positions(): valuing the still-held
    shares is the caller's job, with prices the caller fetched itself. That is
    the "separate live-price path" get_open_positions()'s own docstring said to
    add when it was ever needed.
    """
    with _db() as conn:
        rows = conn.execute("SELECT * FROM positions ORDER BY entry_date ASC, id ASC").fetchall()
        result = []
        for row in rows:
            d = dict(row)
            exit_rows = conn.execute(
                "SELECT exit_price, exit_qty, commission FROM exits WHERE position_id=? ORDER BY id ASC",
                (d["id"],),
            ).fetchall()
            entry_price = d["entry_price"]
            gross = sum((e["exit_price"] - entry_price) * e["exit_qty"] for e in exit_rows)
            fees = (d["entry_commission"] or 0.0) + sum((e["commission"] or 0.0) for e in exit_rows)
            sold_qty = sum(e["exit_qty"] for e in exit_rows)
            result.append({
                "id": d["id"],
                "ticker": d["ticker"],
                "sleeve": get_sleeve(d["ticker"]),
                "status": d["status"],
                "entry_date": d["entry_date"],
                "entry_price": entry_price,
                "qty": d["qty"],
                "sold_qty": sold_qty,
                "remaining_qty": d["qty"] - sold_qty,
                "realized_usd": gross - fees,
                "fees_usd": fees,
            })
    return result
...
# A shadow row is settled when nothing further can happen to it: the simulated
# position is out, or the plan spent its whole waiting window without firing.
# Deliberately derived rather than stored -- a second flag would have to be kept
# in step with the resolution it is derived from, and the two would drift.
#
# Note what is NOT here: a plan that fired and is still running never settles on
# a clock. It runs to a real ending however long that takes (owner, 2026-08-30).
SETTLED_SQL = ("is_fully_closed = 1 OR resolution IN "
               "('expired_never_fired', 'not_testable')")
...
def get_shadow_candidates(ticker: Optional[str] = None) -> list[dict]:
    ...
    Superseded builds are scored too -- see the comment on the query below for
    the numbers that overturned the previous reasoning. A build leaves this list
    when it is SETTLED, not when it is replaced.
    ...
    """
    # Superseded builds are scored too, as of 2026-08-30. They used to be
    # excluded on the reasoning that a replaced plan is a plan the user withdrew,
    # so re-simulating it extends something already abandoned.
    #
    # The data says that reasoning does not hold. All 190 replacements were made
    # by the nightly /screener refresh, the median build survived TWO DAYS, and
    # in 90 of 185 rebuilds the trigger did not move at all. "Superseded" here
    # overwhelmingly means "looked at it again the next night", not "gave up on
    # it" -- and excluding on that basis threw away 190 of 258 ideas, including
    # 44 that were mid-trade and simply froze where they stopped.
    #
    # What ends a row's life is being settled, not being replaced: the position
    # is out, or the plan waited three months without firing. Both are computed
    # below rather than stored, so there is no second flag to keep in step with
    # the resolution it would be derived from.
    sql = "SELECT * FROM ideas WHERE primary_setup IS NOT NULL"
    params: list = []
    if ticker:
        sql += " AND ticker=?"
        params.append(ticker.upper())
    with _db() as conn:
        rows = conn.execute(sql, params).fetchall()
        # A trade with nothing left in it does not change again, and re-running
        # it every night was the whole reason the book looked bigger than it is:
        # UPS stopped out on 2026-07-17 and collected 18 identical rows, CRDO 18,
        # TYL 18. 104 "finished" rows were 13 real trades. The rows already
        # written are kept -- they are a legitimate record of what was known on
        # each night -- but nothing new is added once the position is out.
        finished = {(r["idea_id"], r["setup_side"]) for r in conn.execute(
            "SELECT DISTINCT idea_id, setup_side FROM shadow_outcomes "
            f"WHERE idea_id IS NOT NULL AND ({SETTLED_SQL})"
        )}
    result = []
    for row in rows:
        ...
        if isinstance(d.get("rubric_inputs"), str):
            try:
                d["rubric_inputs"] = json.loads(d["rubric_inputs"])
            except (ValueError, TypeError):
                d["rubric_inputs"] = None
        # Which setups of this build are already done, so a caller scoring both
        # sides can skip the finished one without re-deriving this.
        d["finished_setup_sides"] = sorted(
            side for (iid, side) in finished if iid == d["id"]
        )
        if all(side in d["finished_setup_sides"] for side in SHADOW_SIDES):
            continue      # both setups are done; nothing left on this build
        result.append(d)
    return result


SHADOW_SIDES = ("primary", "alternate")


def get_shadow_trades(include_open: bool = True) -> dict:
    """The shadow book as TRADES rather than as rows.

    The table is append-only by design: one row per idea per setup per night, so
    a trade playing out over three weeks leaves fifteen rows, each a true record
    of what was known that evening. That is worth keeping and it is not a
    dataset -- counting those rows counts nights, not trades, and doing so
    turned 13 real finished trades into "104 closed trades" in an analysis
    earlier today.

    So: the LAST row per (idea_id, setup_side) is the trade. Nothing is deleted
    to achieve that; the grouping happens at read time, here, once, so no caller
    has to remember to do it.

    Rows with no idea_id cannot be grouped at all -- they predate the column and
    carry only a ticker, which is not an identity when a symbol can be screened
    three times. They are counted and reported rather than silently dropped.

    Returns {"trades": [...], "ungrouped": int, "open": int, "closed": int}.
    """
    with _db() as conn:
        rows = [dict(r) for r in conn.execute(
            "SELECT * FROM shadow_outcomes WHERE idea_id IS NOT NULL "
            "ORDER BY idea_id, setup_side, checked_date, id"
        )]
        ungrouped = conn.execute(
            "SELECT COUNT(*) FROM shadow_outcomes WHERE idea_id IS NULL"
        ).fetchone()[0]

    latest: dict = {}
    for row in rows:
        latest[(row["idea_id"], row["setup_side"])] = row     # ordered, so last wins
    trades = list(latest.values())
    if not include_open:
        trades = [t for t in trades if t["is_fully_closed"]]
    closed = sum(1 for t in trades if t["is_fully_closed"])
    return {
        "trades": trades,
        "ungrouped": ungrouped,
        "closed": closed,
...
_SHADOW_SIM_COLUMNS = (
    ...
    "setup_side",       # 2026-08-30: 'primary' or 'alternate'
    "is_fully_closed",  # 2026-08-30: nothing left in the position
    "closest_approach_pct", "closest_approach_atr", "closest_approach_date",
    "move_without_entry_pct",
    "grade_at_fire", "rr_at_fire",
