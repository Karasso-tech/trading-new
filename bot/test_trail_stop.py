...
class TestFrozenAtrIsUsed:
    ...
    def _payload(self, atr_at_build, live_atr):
        return {
            "ticker": "TEST", "sleeve": "swing", "current_price": 106.0, "atr14": live_atr,
    ...
    def test_no_open_position_is_an_honest_error_not_a_stop(self):
        out = trail_stop.compute({"ticker": "TEST", "sleeve": "swing", "current_price": 106.0})
        assert "error" in out
...
class TestOutputShape:
    def test_it_reports_both_numbers_and_the_level_behind_them(self):
        out = trail_stop.compute({
            "ticker": "TEST", "sleeve": "swing", "current_price": 106.0, "atr14": 2.0,
            "swing_lows_recent": LOWS,
...
class TestOlderLevelsDoNotMoveTheStopOnTheirOwn:
    ...
    "the trade has made a new higher low".

    Until 2026-08-30 this was a caution printed next to the raise, and the raise
    happened anyway -- two statements in one message pulling opposite ways.
    Now the level is named and dated, and moving the stop to it is a decision a
    person makes rather than something that happens on its own."""

    def _payload(self, basis_date, entry_date):
        return {
            "ticker": "ASTS", "sleeve": "swing", "current_price": 71.94, "atr14": 6.39,
    ...
    def test_a_pre_entry_low_does_not_raise_the_stop(self):
        # The real ASTS numbers: a 66.53 raise offered off an April low, on a
        # position opened in July.
        out = trail_stop.compute(self._payload("2026-04-29", "2026-07-31"))
        assert out["moved"] is False
        assert out["stop_should_be"] == 54.78
        assert out["held_on_pre_entry_structure"] is True

    def test_the_level_is_still_named_and_dated_not_hidden(self):
        # Held back, never swallowed: a reader can still act on it deliberately.
        out = trail_stop.compute(self._payload("2026-04-29", "2026-07-31"))
        assert out["basis_after_entry"] is False
        assert out["stop_basis_level"] == 67.49
        assert out["stop_basis_date"] == "2026-04-29"
        assert "BEFORE this position was opened" in out["caution"]

    def test_a_low_made_since_entry_still_raises_the_stop_normally(self):
        out = trail_stop.compute(self._payload("2026-08-04", "2026-07-31"))
        assert out["basis_after_entry"] is True
        assert out["moved"] is True
        assert out["stop_should_be"] == pytest.approx(66.5315)
        assert out["held_on_pre_entry_structure"] is False
        assert "caution" not in out

    def test_the_date_is_the_only_difference_between_the_two(self):
        # Identical levels, identical arithmetic -- only when the low was made
        # decides whether it acts by itself.
        held = trail_stop.compute(self._payload("2026-04-29", "2026-07-31"))
        fresh = trail_stop.compute(self._payload("2026-08-04", "2026-07-31"))
        assert held["stop_basis_level"] == fresh["stop_basis_level"]
        assert held["moved"] is not fresh["moved"]
...
class TestCoreIsNeverTrailed:
    """CONSISTENCY_RULES.md rule 8, and the trap the Runner-only unlock set.

    A Core holding has no targets, so build_tranche_plan marks it `runner_only`
    for the identical mechanical reason a rule-6 swing position is marked so.
    The live book really does hold two of them -- SPY and QQQ -- and without
    this guard the unlock would have started ratcheting the structural stop
    under exactly the two positions rule 8 says not to manage at all.
    """

    def _payload(self, sleeve):
        return {
            "ticker": "QQQ", "sleeve": sleeve, "current_price": 106.0, "atr14": ATR,
            "swing_lows_recent": LOWS,
            "open_position": {
                "current_stop": 95.0, "entry_price": 100.0, "initial_stop": 95.0,
                "entry_date": "2026-07-01",
                "entry_setup": {"atr_at_build": ATR},
                "tranche_plan": {"runner_only": True,
                                  "tranches": [{"label": "runner", "filled_qty": 0,
                                                 "status": "open"}]},
            },
        }

    def test_a_core_holding_is_left_alone(self):
        out = trail_stop.compute(self._payload("core"))
        assert out["moved"] is False
        assert out["stop_should_be"] == 95.0
        assert "rule 8" in out["reason"]

    def test_an_unknown_sleeve_is_left_alone_too(self):
        # "If unclear, ask instead of assume" -- the rule sleeve.py exists for.
        out = trail_stop.compute(self._payload("unknown"))
        assert out["moved"] is False

    def test_the_identical_position_trails_once_it_is_marked_swing(self):
        # Proof the sleeve is what stopped it, not the levels.
        out = trail_stop.compute(self._payload("swing"))
        assert out["moved"] is True
...
class TestNoTrailingBeforeTargetOne:
    ...
    def _payload(self, tranches):
        return {
            "ticker": "TEST", "sleeve": "swing", "current_price": 106.0, "atr14": ATR,
            "swing_lows_recent": LOWS,
            "open_position": {
                "current_stop": 95.0, "entry_price": 100.0, "initial_stop": 95.0,
                "entry_date": "2026-07-01",
    ...
    def test_a_runner_only_position_trails_because_it_has_no_target_to_wait_for(self):
        # Rule 6: no qualifying target anywhere, so the whole position is a
        # Runner and the trailing stop is the ENTIRE exit method. The old guard
        # waited for a numbered target to fill, and a Runner-only plan has no
        # numbered target to fill -- so its stop could never move at all, on the
        # one kind of position where the stop moving IS the plan.
        out = trail_stop.compute(self._payload_runner_only())
        assert out["runner_only"] is True
        assert out["past_target_1"] is True
        assert out["moved"] is True
        assert out["stop_should_be"] == pytest.approx(102.7)

    def test_runner_only_still_obeys_the_noise_floor(self):
        # The unlock is not a loosening. At price 104 the 103 low is inside the
        # 1.4 noise band, exactly as for any other position, so the trail falls
        # back to the next real low down rather than using the closest one.
        payload = self._payload_runner_only()
        payload["current_price"] = 104.0
        out = trail_stop.compute(payload)
        assert out["stop_basis_level"] == 96.0

    def test_runner_only_still_never_moves_the_stop_down(self):
        payload = self._payload_runner_only()
        payload["open_position"]["current_stop"] = 104.0
        out = trail_stop.compute(payload)
        assert out["moved"] is False
        assert out["stop_should_be"] == 104.0

    def _payload_runner_only(self):
        # What build_tranche_plan really returns for a rule-6 position: the
        # runner row is always present, at 100%, and there is no target row.
        payload = self._payload([{"label": "runner", "filled_qty": 0, "status": "open"}])
        payload["open_position"]["tranche_plan"]["runner_only"] = True
        return payload
