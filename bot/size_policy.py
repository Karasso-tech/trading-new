...
def format_size_lines_he(result: SizeResult, risk_usd_target: float,
                          ...
                          swing_usd: Optional[float] = None) -> str:
    ...
    # The same dollars against the money that is actually being traded with
    # (2026-08-30). Risk has always been quoted as a percentage of the WHOLE
    # account, and roughly 60% of the account is Core -- SPY/QQQ held as an
    # allocation, exempt from all of this under rule 8 and never sold to fund a
    # swing entry. So 1% of the account is 2.5% of the swing sleeve, and six
    # open positions inside the 6% heat cap are 15% of it.
    #
    # Both numbers are true and they answer different questions. This shows
    # both and changes neither: rule 28's floor, ceiling and target are all
    # still measured against the account figure, exactly as before. Which
    # denominator SHOULD size a trade is an open question with a measurement
    # waiting to answer it, and this line is not that answer -- it just stops
    # the smaller number from being the only one on screen.
    pct_of_swing = (result.risk_usd_actual / swing_usd * 100) if swing_usd else None
    ...
    if pct_of_equity is not None and pct_of_swing is not None:
        risk_line += (f" — <b>{pct_of_equity:.2f}%</b> מכל התיק, "
                      f"<b>{pct_of_swing:.2f}%</b> מכסף הסווינג")
    elif pct_of_equity is not None:
