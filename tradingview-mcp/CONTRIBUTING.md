...
This tool is a **local bridge** between Claude Code and a TradingView chart tab open in a dedicated Chrome window on your machine. All contributions must stay within this scope.
...
- UI automation for the locally running Chrome tab

### What's out of scope

Contributions **must not** add features that:

- **Connect directly to TradingView's servers** — all data access must go through the locally running Chrome tab via CDP
...
tv status         # verify CDP connection (Chrome must be running on a TradingView chart tab)
```
...
- Test against a live TradingView chart tab (in Chrome) before submitting
