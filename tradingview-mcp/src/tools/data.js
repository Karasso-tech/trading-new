...
export function registerDataTools(server) {
  server.tool('data_get_ohlcv', 'Get OHLCV bar data from the chart. Use summary=true for compact stats instead of all bars (saves context). Use `before` to page backward through history beyond the 500-bar cap.', {
    ...
    before: z.coerce.number().optional().describe('Unix timestamp (seconds). If set, returns the `count` bars immediately before this time instead of the most recent ones — pass the oldest `time` from a previous page to walk further back. Vendored addition, not upstream.'),
  }, async ({ count, summary, before }) => {
    try { return jsonResult(await core.getOhlcv({ count, summary, before })); }
