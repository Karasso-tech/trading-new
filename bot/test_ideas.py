...
class TestShadowBinding:
    def test_every_unsettled_build_is_a_candidate_carrying_its_own_id(self, temp_db):
        """Changed 2026-08-30: a replaced build is still scored.

        This used to assert that only the newest build is a candidate, on the
        reasoning that a replaced plan is one the user withdrew. The data
        overturned it -- all 190 replacements came from the nightly refresh,
        the median build survived two days, and in 90 of 185 rebuilds the
        trigger did not move at all. Excluding on that basis dropped 190 of 258
        ideas, 44 of them mid-trade.

        What removes a build from the list is being settled, not being replaced.
        """
        older = _save(primary_setup=_setup(100.0, 95.0))
        newest = _save(primary_setup=_setup(58.41, 56.89))
        candidates = persistence.get_shadow_candidates()

        assert {c["idea_id"] for c in candidates} == {older, newest}
        # each still carries its OWN plan -- the whole point of idea_id
        by_id = {c["idea_id"]: c for c in candidates}
        assert by_id[older]["primary_setup"]["trigger"] == 100.0
        assert by_id[newest]["primary_setup"]["trigger"] == 58.41
        assert by_id[newest]["date_built"] == by_id[newest]["built_at"]

    def test_a_settled_build_drops_out_however_it_settled(self, temp_db):
        # Two ways to be done, and neither of them is "was replaced": the
        # position is out, or the plan waited its whole window without firing.
        out = _save(primary_setup=_setup(100.0, 95.0))
        waited = _save(primary_setup=_setup(58.41, 56.89))
        # Both setups of both builds, as of 2026-08-30: a build is done when
        # everything on it is done, not when its Primary is.
        for idea_id, resolution, closed in ((out, "stop", 1),
                                             (waited, "expired_never_fired", 0)):
            for side in ("primary", "alternate"):
                persistence.record_shadow_outcome(
                    "XLF", checked_date="2026-08-30", price=99.0,
                    hypothetical_trigger_fired=bool(closed),
                    max_favorable_excursion=None, max_adverse_excursion=None,
                    idea_id=idea_id, setup_side=side,
                    resolution=resolution, is_fully_closed=closed)
